...
#define LLC_WRITE_OVERHEAD 20  // 4/5 (L1I or L1D) + 10 + 40 = 54/55 cycles
#define READ_MISS_BYPASS 1   //Read miss Bypass
#define RUN_AHEAD 0	        //Run Ahead
#define L2C_WRITE_MERGE 1    //L2 write merge
#define WRITE_BYPASS 1	       //Write Bypass
#define DRAM_RQ_OPTIMIZATION 0	//drop the packet from dram rq if it is in LLC

#define LLC_BANK 1
#define WRITE_THR 32
#define LIVE_WRITE_THR 15 //Prabuddha sir no need
...
class CACHE : public MEMORY {
  public:
    ...
    uint8_t cache_type;
    uint64_t tRa, cRa;	//tRa = total run ahead done, cRa = correct prediction
    ...
    // Exclusive LLC counters
    uint64_t llc_exclusive_invalidations;  // blocks invalidated from LLC after sending to L2
    ...
    void add_mshr(PACKET *packet),
         update_fill_cycle(),
         l2c_initialize_replacement(),                        // Modified by Prathamesh
         ...
         l2c_update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, PACKET *blk, uint64_t victim_addr, uint32_t type, uint8_t hit),    // Modified by Prathamesh
         llc_update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, PACKET *pack, uint64_t victim_addr, uint32_t type, uint8_t hit),
         lru_update(uint32_t set, uint32_t way),
         fill_cache(uint32_t set, uint32_t way, PACKET *packet),
         replacement_final_stats(),
         l2c_replacement_final_stats(),      // Modified by Prathamesh
    ...
    uint32_t get_set(uint64_t address),
             ...
             llc_find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, PACKET *pack, uint64_t full_addr, uint32_t type),
             l2c_find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type),  // Modified by Prathamesh
