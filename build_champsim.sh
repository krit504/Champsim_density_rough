...
if [ "$#" -ne 8 ]; then
    ...
    echo "Usage: ./build_champsim.sh [branch_pred] [l1i_pref] [l1d_pref] [l2c_pref] [llc_pref] [l2c_repl] [llc_repl] [num_core]"
    exit 1
fi
...
LLC_PREFETCHER=$5 
L2C_REPLACEMENT=$6  # prefetcher/*.llc_pref
LLC_REPLACEMENT=$7  # replacement/*.llc_repl
NUM_CORE=$8         # tested up to 8-core system
...
echo "Cores: ${NUM_CORE}"
BINARY_NAME="${BRANCH}-${L1I_PREFETCHER}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${LLC_PREFETCHER}-${L2C_REPLACEMENT}-${LLC_REPLACEMENT}-${NUM_CORE}core"
