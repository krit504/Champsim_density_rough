...
void MEMORY_CONTROLLER::schedule(PACKET_QUEUE *queue)
{
    ...
    if (oldest_index != -1) { // scheduler might not find anything if all requests are already scheduled or all banks are busy
        ...
        int LATENCY = 0;
        if(queue->entry[oldest_index].run_ahead)
        {
	    if(uncore.LLC.check_hit(&queue->entry[oldest_index])==-1){
	     	if (row_buffer_hit )  
		    LATENCY = tCAS - uncore.LLC.LATENCY - queue->entry[oldest_index].ls_gt*LLC_WRITE_OVERHEAD;
		else 
		    LATENCY = tRP + tRCD + tCAS - uncore.LLC.LATENCY - queue->entry[oldest_index].ls_gt*LLC_WRITE_OVERHEAD;
	    }
	    else if(!DRAM_RQ_OPTIMIZATION){
	    	if (row_buffer_hit)  
		    LATENCY = tCAS;
		else 
...
int MEMORY_CONTROLLER::add_rq(PACKET *packet)
{
    ...
    // check for duplicates in the read queue                                                  // Important for
    int index = check_dram_queue(&RQ[channel], packet);
    if (index != -1){
  	/*if(packet->run_ahead)*/ RQ[channel].MERGED++;  
