...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
	      ...
	      if(fill_level == FILL_L2)
		{
	      ...
	      else if(MSHR.entry[mshr_index].bypass_done!=1)
		{
...
{
  uint64_t global_start = find_non_overlapping_global_cycle(0, packet->cpu, 0);
  RQ.entry[index].event_cycle = current_core_cycle[packet->cpu] + (global_start + 0 - GLOBAL_CYCLE);
}
else
{
  if(rs==0)
  {   
    uint64_t global_start = find_non_overlapping_global_cycle(LATENCY+15, packet->cpu, 0);
    RQ.entry[index].event_cycle = current_core_cycle[packet->cpu] + (global_start + LATENCY+15 - GLOBAL_CYCLE);
      rs=1;
  }
  else
  {
    uint64_t global_start = find_non_overlapping_global_cycle(LATENCY, packet->cpu, 0);
    RQ.entry[index].event_cycle = current_core_cycle[packet->cpu] + (global_start + LATENCY - GLOBAL_CYCLE);
  }
}
}
 else{
...
else
{
  if(ws==0)
  {
    ...
    uint64_t global_start = find_non_overlapping_global_cycle(WRITE_LATENCY+20, packet->cpu, 1);
    WQ.entry[index].event_cycle = current_core_cycle[packet->cpu] + (global_start + WRITE_LATENCY+20 - GLOBAL_CYCLE);
    ...
    if(cache_type==IS_LLC && all_warmup_complete>NUM_CPUS)
{
 ...
 {
       if (MSHR.entry[mshr_index].instruction)
		    upper_level_icache[packet->cpu]->return_data(&MSHR.entry[mshr_index]);
		  if (MSHR.entry[mshr_index].is_data)
                    upper_level_dcache[packet->cpu]->return_data(&MSHR.entry[mshr_index]);
      MSHR.entry[mshr_index].bypass_done=1;
 }
}
...
else
{
    ...
    {
        uint64_t global_start = find_non_overlapping_global_cycle(WRITE_LATENCY+20, packet->cpu, 1);
    MSHR.entry[mshr_index].event_cycle = current_core_cycle[packet->cpu] + (global_start + WRITE_LATENCY+20 - GLOBAL_CYCLE);
     fs=1;  
  }   
  else
  {
...
uint64_t CACHE::find_non_overlapping_global_cycle(uint64_t latency, int core_id, bool is_write) {
   ...
   //port_schedule_table.push_back(CACHE::PortScheduleEntry{start, end, core_id, (int)is_write});
