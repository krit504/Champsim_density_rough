...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if (cache_type == IS_LLC) {
            way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], &MSHR.entry[mshr_index], MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
        }
        else if (cache_type == IS_L2C)          // Modified by Prathamesh
        {
...
//#ifdef LLC_BYPASS
        if ((cache_type == IS_LLC) && (way == LLC_WAY)) { // this is a bypass that does not fill the LLC
            ...
            if (cache_type == IS_LLC) {
                llc_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, &MSHR.entry[mshr_index], 0, MSHR.entry[mshr_index].type, 0);

            }
            else if (cache_type == IS_L2C)               // Modified by Prathamesh
            {
                l2c_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, &MSHR.entry[mshr_index], 0, MSHR.entry[mshr_index].type, 0);
            }
            else
...
//#endif
        ...
        if (block[set][way].dirty) {
            ...
            if (lower_level) {
                if (lower_level->get_occupancy(2, block[set][way].address) == lower_level->get_size(2, block[set][way].address)) {
                    ...
                    writeback_packet.live_score = block[set][way].live_score;
                    writeback_packet.carry_indicator = block[set][way].carry_indicator;
                    writeback_packet.run_ahead = block[set][way].run_ahead;
                    writeback_packet.is_bypass = block[set][way].is_bypass;
                    writeback_packet.l2c_wm = block[set][way].l2c_wm;
                    writeback_packet.ls_gt = block[set][way].ls_gt;
	  ...
	  if (cache_type == IS_L1I)
		...
		cpu = 0;
            ...
            // update replacement policy
            if (cache_type == IS_LLC) {
                llc_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, &MSHR.entry[mshr_index], block[set][way].full_addr, MSHR.entry[mshr_index].type, 0);
                //
               // temp_cycle[fill_cpu] += LLC_WRITE_OVERHEAD;
            }
            else if (cache_type == IS_L2C)               // Modified by Prathamesh
            {
                l2c_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, &MSHR.entry[mshr_index], block[set][way].full_addr, MSHR.entry[mshr_index].type, 0);
            }
            else
            ...
            // check fill level                 (This is implemented in a different way in both codes MTP1 and  Part1)
            if (MSHR.entry[mshr_index].fill_level < fill_level) {
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        int bypass = llc_find_victim(writeback_cpu, WQ.entry[index].instr_id, set, block[set], &WQ.entry[index], WQ.entry[index].full_addr, WQ.entry[index].type);
        int way = check_hit(&WQ.entry[index]);
        //int lscore = WQ.entry[index].live_score;
        
#ifdef LLC_BYPASS
	    if ((cache_type == IS_LLC) && (bypass == LLC_WAY)) { // this is a bypass that does not fill the LLC

		//WQ.entry[index].is_bypass=1;
		
            // update replacement policy
            if(way>=0){
		    if (cache_type == IS_LLC) {
		        llc_update_replacement_state(writeback_cpu, set, way, WQ.entry[index].full_addr, &WQ.entry[index], 0, WQ.entry[index].type, 1);

		    }
		    else if (cache_type == IS_L2C)               // Modified by Prathamesh
		    {
		        l2c_update_replacement_state(writeback_cpu, set, way, WQ.entry[index].full_addr, &WQ.entry[index], 0, WQ.entry[index].type,1);
		    }
		    else
		        update_replacement_state(writeback_cpu, set, way, WQ.entry[index].full_addr, WQ.entry[index].ip, 0, WQ.entry[index].type, 1);
            }
            ...
            //fill WQ of Main Memory.
            lower_level->add_wq(&WQ.entry[index]);
            

	    if(warmup_complete[writeback_cpu] && (WQ.entry[index].cycle_enqueued != 0))
	      {
		uint64_t current_miss_latency = (current_core_cycle[writeback_cpu] - WQ.entry[index].cycle_enqueued);
		total_miss_latency += current_miss_latency;
	      }
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            if (cache_type == IS_LLC) {
                llc_update_replacement_state(writeback_cpu, set, way, block[set][way].full_addr, &WQ.entry[index], 0, WQ.entry[index].type, 1);
                //
                //temp_cycle[writeback_cpu] += LLC_WRITE_OVERHEAD;

            }
            else if (cache_type == IS_L2C)
            {
                l2c_update_replacement_state(writeback_cpu, set, way, block[set][way].full_addr, &WQ.entry[index], 0, WQ.entry[index].type, 1);
            }
            else
		...
		if(mshr_index == -2)
		  ...
		  if(cache_type == IS_LLC)                 // some confusion here
		    {
			...
			if((WQ.entry[index].fill_l1d) && (MSHR.entry[mshr_index].fill_l1d != 1))
			  {
                ...
                if (cache_type == IS_LLC) {
                    way = llc_find_victim(writeback_cpu, WQ.entry[index].instr_id, set, block[set], &WQ.entry[index], WQ.entry[index].full_addr, WQ.entry[index].type);
                }
                else if (cache_type == IS_L2C)                   // Modified by Prathamesh
                {
...
/*
		  ...
		  if (cache_type == IS_L1I)
			...
			cpu = 0;
                    ...
                    // update replacement policy
                    if (cache_type == IS_LLC) {
                        llc_update_replacement_state(writeback_cpu, set, way, WQ.entry[index].full_addr, &WQ.entry[index], block[set][way].full_addr, WQ.entry[index].type, 0);
                        //
                	//temp_cycle[writeback_cpu] += LLC_WRITE_OVERHEAD;
                    ...
                    {
                        l2c_update_replacement_state(writeback_cpu, set, way, WQ.entry[index].full_addr, &WQ.entry[index], block[set][way].full_addr, WQ.entry[index].type, 0);
                    }
                    else
      ...
      if (read_cpu == NUM_CPUS)
		...
		if (RQ.entry[index].type == LOAD) {
			...
			cpu = 0;
                ...
                // update replacement policy
                if (cache_type == IS_LLC) {
                    llc_update_replacement_state(read_cpu, set, way, block[set][way].full_addr, &RQ.entry[index], 0, RQ.entry[index].type, 1);

                }
                else if (cache_type == IS_L2C)          //Modified by Prathamesh
                {
                    l2c_update_replacement_state(read_cpu, set, way, block[set][way].full_addr, &RQ.entry[index], 0, RQ.entry[index].type, 1);
                }
                else
		...
		if (cache_type == IS_L2C)          //Modified by Prathamesh
                {
                    l2c_update_replacement_state(read_cpu, set, L2C_WAY, 0, &RQ.entry[index], 0, RQ.entry[index].type, 0);
                }
                else if (cache_type == IS_LLC)          //Modified by Prathamesh
                {
                    llc_update_replacement_state(read_cpu, set, LLC_WAY, 0, &RQ.entry[index], 0, RQ.entry[index].type, 0);
                }
		if(mshr_index == -2)
		  {
		    // this is a data/instruction collision in the MSHR, so we have to wait before we can allocate this miss
		  ...
		  if(cache_type == IS_LLC)
		      ...
		      else
			{
			  ...
			  if(lower_level)
			    {
			      tmp_dram_index = lower_level->add_rq(&RQ.entry[index]);
			    }
			    
			    /*if(tmp_dram_index>=0){
			    	    int tmp_mshr_index = check_mshr(&RQ.entry[index]);
			     	    uint32_t channel = uncore.DRAM.dram_get_channel(RQ.entry[index].address);
			     	    
		                    uint8_t  prior_returned = uncore.DRAM.RQ[channel].entry[tmp_dram_index].returned;
		                    uint64_t prior_event_cycle = uncore.DRAM.RQ[channel].entry[tmp_dram_index].event_cycle;
		                    
		                    // in case request is already returned, we should keep event_cycle and retunred variables
		                    MSHR.entry[tmp_mshr_index].returned = prior_returned;
		                    variable = MSHR.entry[tmp_mshr_index].event_cycle - prior_event_cycle;
		          
                            }*/
			}
		    }
		    else if(cache_type == IS_L2C && RQ.entry[index].run_ahead)
		    {
		      ...
		      if (lower_level){
                        ...
                        //if(!lower_level->get_occupancy(0, RQ.entry[index].address) < lower_level->get_size(0, RQ.entry[index].address)) lower_level->MSHR.add_queue(&RQ.entry[index]);
		                if (lower_level->lower_level->get_occupancy(1, RQ.entry[index].address) < lower_level->lower_level->get_size(1, RQ.entry[index].address))
				{
					
				        if(lower_level->lower_level) {
				        	uncore.LLC.tRa++;
				        	lower_level->lower_level->add_rq(&RQ.entry[index]);
				        	if(uncore.LLC.check_hit(&RQ.entry[index])==-1) uncore.LLC.cRa++;
				        }
				}
			}
      ...
      if (prefetch_cpu == NUM_CPUS)
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
            ...
            if (way >= 0) { // prefetch hit
                ...
                // update replacement policy
                if (cache_type == IS_LLC) {
                    llc_update_replacement_state(prefetch_cpu, set, way, block[set][way].full_addr, &PQ.entry[index], 0, PQ.entry[index].type, 1);

                }
                else if (cache_type == IS_L2C)   // Modified by Prathamesh
                {
                    l2c_update_replacement_state(prefetch_cpu, set, way, block[set][way].full_addr, &PQ.entry[index], 0, PQ.entry[index].type, 1);
                }
                else
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
{
    ...
    block[set][way].live_score = packet->live_score;
    if(cache_type != IS_L2C) block[set][way].run_ahead = packet->run_ahead;
    block[set][way].carry_indicator = packet->carry_indicator;
    block[set][way].is_bypass = packet->is_bypass;
    block[set][way].l2c_wm = packet->l2c_wm;
    block[set][way].ls_gt = packet->ls_gt;
...
int CACHE::add_rq(PACKET *packet)
{
    ...
    if (wq_index != -1) {
        ...
        if ((cache_type == IS_L1D) && (packet->type != PREFETCH)) {
            ...
            DP ( if (warmup_complete[packet->cpu]) {
            cout << "[" << NAME << "_] " << __func__ << " instr_id: " << packet->instr_id << " found recent writebacks";
    ...
    // ADD LATENCY
   /* if(cache_type == IS_LLC && check_hit(&RQ.entry[index]) >= 0 && packet->run_ahead)
    {
    			//count2++;
		    	//cout<<"Count2: "<<count2<<endl;
    
        if (RQ.entry[index].event_cycle < current_core_cycle[packet->cpu])
        RQ.entry[index].event_cycle = current_core_cycle[packet->cpu] + LATENCY - (tCAS);
    else
        RQ.entry[index].event_cycle -= (tCAS-LATENCY);
    }
    else
    {*/
        if (RQ.entry[index].event_cycle < current_core_cycle[packet->cpu])
...
int CACHE::add_wq(PACKET *packet)
{
    ...
    if(cache_type == IS_LLC)
    {
        if (WQ.entry[index].event_cycle < current_core_cycle[packet->cpu])
            WQ.entry[index].event_cycle = current_core_cycle[packet->cpu] + LATENCY + LLC_WRITE_OVERHEAD;
        else
            WQ.entry[index].event_cycle += (LATENCY + LLC_WRITE_OVERHEAD);
...
int CACHE::prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int pf_fill_level, uint32_t prefetch_metadata)
{
    ...
    if (PQ.occupancy < PQ.SIZE) {
	    ...
	    pf_packet.pf_metadata = prefetch_metadata;
            ...
            pf_packet.live_score = 0;
            pf_packet.carry_indicator = 0;
            pf_packet.run_ahead = 0;
            pf_packet.is_bypass = 0;
            pf_packet.l2c_wm = 0;
            pf_packet.ls_gt = 0;
...
int CACHE::add_pq(PACKET *packet)
{
    ...
    /*if(cache_type == IS_LLC)
    {
        if (PQ.entry[index].event_cycle < current_core_cycle[packet->cpu])
            PQ.entry[index].event_cycle = current_core_cycle[packet->cpu] + READ_LATENCY;                   // Modified by Prathamesh
        else
            PQ.entry[index].event_cycle += READ_LATENCY;
    }
    else
    {*/
        if (PQ.entry[index].event_cycle < current_core_cycle[packet->cpu])
...
void CACHE::return_data(PACKET *packet)
{
    ...
    MSHR.entry[mshr_index].live_score = packet->live_score;
    MSHR.entry[mshr_index].run_ahead = packet->run_ahead;
    MSHR.entry[mshr_index].carry_indicator = packet->carry_indicator;
    MSHR.entry[mshr_index].is_bypass = packet->is_bypass;
    MSHR.entry[mshr_index].l2c_wm = packet->l2c_wm;
    MSHR.entry[mshr_index].ls_gt = packet->ls_gt;
    ...
    if(cache_type == IS_LLC)
    {
		...
		if (MSHR.entry[mshr_index].event_cycle < current_core_cycle[packet->cpu])
		    MSHR.entry[mshr_index].event_cycle = current_core_cycle[packet->cpu] + 0;
		else
		    MSHR.entry[mshr_index].event_cycle +=0;
	}
	else{*/
		if (MSHR.entry[mshr_index].event_cycle < current_core_cycle[packet->cpu])
		    MSHR.entry[mshr_index].event_cycle = current_core_cycle[packet->cpu] + LATENCY + LLC_WRITE_OVERHEAD;
		else
		    MSHR.entry[mshr_index].event_cycle += (LATENCY + LLC_WRITE_OVERHEAD);
	//}
    }
    else
    {
